...
export default function Painel({ clientes, salvas, cfg, onNavigate, onNovoCliente, onNovaReuniao }) {
  const aprendizados = clientes.reduce((n, c) => n + (c.aprendizados || []).length, 0);
  const recentes = salvas.slice(0, 8);
  const totalAgentes = Object.keys(AGENTS).length - 1;
